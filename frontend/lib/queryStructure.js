...
const parseCifAtoms = (input) => {
    ...
    for (let i = 0; i < lines.length; i += 1) {
        ...
        const chainIdx = [idx('_atom_site.auth_asym_id'), idx('_atom_site.label_asym_id')].find(v => v >= 0);
        const seqIdx = [idx('_atom_site.auth_seq_id'), idx('_atom_site.label_seq_id')].find(v => v >= 0);
        const compIdx = [idx('_atom_site.auth_comp_id'), idx('_atom_site.label_comp_id')].find(v => v >= 0);
        const atomIdx = [idx('_atom_site.auth_atom_id'), idx('_atom_site.label_atom_id')].find(v => v >= 0);
        ...
        while (j < lines.length) {
            ...
            if (line === 'loop_' || line.startsWith('data_') || line.startsWith('_')) {
                j -= 1;
                break;
            }
            if (line.startsWith(';')) {
...
const createQueryStructure = (parsedAtoms) => {
    ...
    return {
        ...
        getAtomSetWithinSelection(selection, cutoff) {
            const selected = selection instanceof Set
                ? selection
            ...
            const selectedAtoms = Array.from(selected).map(idx => atoms[idx]).filter(Boolean);
            const near = new Set();
...
export function parseQueryStructure(input) {
    ...
    const parsedAtoms = (trimmed[0] === '#' || trimmed.startsWith('data_'))
        ? parseCifAtoms(input)
        : parsePdbAtoms(input);
